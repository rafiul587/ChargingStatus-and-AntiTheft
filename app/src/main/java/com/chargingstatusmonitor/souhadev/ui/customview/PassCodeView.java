package com.chargingstatusmonitor.souhadev.ui.customview;
...
public class PassCodeView extends View {
  ...
  private void preparePaint() {
    ...
    circlePaint.setColor(ResourcesCompat.getColor(getResources(), R.color.green_700, null));
  ...
  private void drawKeyPad(Canvas canvas) {
    ...
    for (KeyRect rect : keyRects) {
      if(!rect.value.isEmpty()) {
        canvas.drawCircle(rect.rect.exactCenterX(), rect.rect.exactCenterY(), 48,
                circlePaint);
      }
      canvas.drawText(rect.value, rect.rect.exactCenterX(), rect.rect.exactCenterY() - centerHalf,
              textPaint);
      if (rect.hasRippleEffect) {
        ripplePaint.setAlpha(rect.circleAlpha);
  ...
  private void notifyListener(String currentPassCode) {
    if (textChangeListener != null) {
      textChangeListener.onTextChanged(currentPassCode);
    }
  }
